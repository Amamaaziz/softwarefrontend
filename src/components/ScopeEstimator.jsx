...
// scope and drives a live Complexity Gauge. The whole thing sits inside the
// ElectricBorder canvas: as complexity climbs, the border's `speed` and
// `chaos` props ramp up so the frame literally pulses with more energy.
//
// NOTE: adjust the ElectricBorder import path if it lives outside components/ui/.
...
export default function ScopeEstimator() {
  ...
  // The twist: complexity drives the electric border's energy
  const borderSpeed = 0.6 + ratio * 2.6
  const borderChaos = 0.06 + ratio * 0.22
  ...
  return (
    <ElectricBorder color="#00D2C4" speed={borderSpeed} chaos={borderChaos} borderRadius={24}>
      <div className="rounded-[24px] bg-[#0a1c1b] p-6 text-slate-300 sm:p-10">
        {/* Step indicator */}
        ...
        <div className="mt-8 min-h-[13rem]">
          ...
          {step === 2 && (
            <>
              ...
              <p className="mt-1 text-sm text-slate-400">Watch the gauge — and the border — react.</p>
